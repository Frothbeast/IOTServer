...
function App() {
    ...
    const columnStats = useMemo(() => {
        ...
        return {
            ...
            datetime: {     avg: StatsLib.avg(datetime.slice(1).map((v, i) => new Date(datetime[i]).getTime() - new Date(v).getTime())),
                            max: StatsLib.max(datetime.slice(1).map((v, i) => new Date(datetime[i]).getTime() - new Date(v).getTime())),
                            min: StatsLib.min(datetime.slice(1).map((v, i) => new Date(datetime[i]).getTime() - new Date(v).getTime()))
                        },
